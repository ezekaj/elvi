          ...
          <StaggeredContainer className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-6xl mx-auto" staggerDelay={0.2}>
            {projects.map((project, index) => (
              <StaggeredItem key={project.id}>
                ...
                <div>
                  <h3 className={`text-xl font-bold ${textPrimary} mb-3`}>{project.title}</h3>
                  <p className={`${textSecondary} leading-relaxed mb-4`}>{project.description}</p>

                  <div className="flex flex-wrap gap-2 mb-4">
                    {project.technologies.map((tech, index) => (
                      <span key={index} className={`px-3 py-1 ${getSkillColors('cyan').bg} ${getSkillColors('cyan').text} rounded-full text-sm border ${getSkillColors('cyan').border}`}>
                        {tech}
                      </span>
                    ))}
                  </div>
                  ...
                  <div>
                    <a href={project.link} className={`inline-flex items-center ${accent} hover:opacity-80 font-medium transition-opacity`}>
                      View Project
          ...
          <AnimatedSection className={`mt-16 ${card} rounded-2xl p-8`} delay={0.6}>
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-center">
              <div>
                ...
                <h2 className={`text-3xl font-bold ${textPrimary} mb-4`}>AI-Powered Portfolio System</h2>
                ...
                <div className="flex flex-wrap gap-2 mb-6">
                  <span className={`px-3 py-1 ${getSkillColors('purple').bg} ${getSkillColors('purple').text} rounded-full text-sm border ${getSkillColors('purple').border}`}>React</span>
                  <span className={`px-3 py-1 ${getSkillColors('blue').bg} ${getSkillColors('blue').text} rounded-full text-sm border ${getSkillColors('blue').border}`}>TypeScript</span>
                  <span className={`px-3 py-1 ${getSkillColors('cyan').bg} ${getSkillColors('cyan').text} rounded-full text-sm border ${getSkillColors('cyan').border}`}>Tailwind</span>
                  <span className={`px-3 py-1 ${getSkillColors('green').bg} ${getSkillColors('green').text} rounded-full text-sm border ${getSkillColors('green').border}`}>Vite</span>
                </div>
                <a href="https://github.com/ezekaj/elvi" className={`inline-flex items-center ${accent} hover:opacity-80 font-medium transition-opacity`}>
                  View Source Code
                  <span className="ml-2">→</span>
                </a>
              </div>
              <div>
                <div className={`${card} rounded-lg p-4 font-mono text-sm`}>
                  <div className="flex items-center mb-3">
                    <div className="w-3 h-3 bg-red-500 rounded-full mr-2"></div>
                    <div className="w-3 h-3 bg-yellow-500 rounded-full mr-2"></div>
                    <div className="w-3 h-3 bg-green-500 rounded-full mr-2"></div>
                    <span className={`${textSecondary} ml-4`}>portfolio.tsx</span>
                  </div>
